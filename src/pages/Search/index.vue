...
export default {
  ...
  data() {
    return {
      ...
      searchParams: {
        ...
        keyword: '', //关键字
        order: '1:desc', //排序
        pageNo: 1, //分页器用的：代表的是当前页是第几页
        pageSize: 3, //代表每一页展示的数据个数
        props: [], //平台售卖属性操作带的参数
        trademark: '' //品牌
      }
    }
  ...
  methods: {
    //向服务器发请求获取seach模块数据（根据参数不同返回不同的数据进行展示）
    ...
    removeCategoryName() {
      ...
      this.getData()
      //地址栏也需要改：进行路由跳转(现在的路由只是跳转到自己这里)
      //严谨：本意是删除query，如果路径中含有params不应该删除，路由跳转的时候应该带着
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      //再次发起ajax请求
      this.getData()
      //每一次请求完毕，应该把相应1,2,3级分类的id置空，让他接收下一次的相应1,2,3id
      this.searchParams.category1Id = undefined
      this.searchParams.category2Id = undefined
      this.searchParams.category3Id = undefined
    }
